...
static AEGfxVertexList* circle_mesh = nullptr;
...
static void AEVec2Rotate(AEVec2* result, AEVec2* input, f32 radians)
{
    AEVec2 r;
    r.x = input->x * cosf(radians) - input->y * sinf(radians);
    r.y = input->x * sinf(radians) + input->y * cosf(radians);
    (*result) = r;
}

static bool circle_to_circle_collision(
    f32 lhs_x, f32 lhs_y, f32 lhs_r,
    f32 rhs_x, f32 rhs_y, f32 rhs_r)
{
    f32 diff_x = lhs_x - rhs_x;
    f32 diff_y = lhs_y - rhs_y;
    f32 dist = sqrtf(diff_x * diff_x + diff_y * diff_y);

    return dist < (lhs_r + rhs_r);
}
...
static void draw_circle(f32 x, f32 y, f32 rad, f32 r, f32 g, f32 b, f32 a)
{
        ...
        _In_ int       nCmdShow)
{
    ...
    {
        const f32 parts = 100;
        const f32 rotation_per_part = (f32)(2*PI/(f32)parts);
        AEGfxMeshStart();
        AEVec2 vec_a;
        AEVec2 vec_b;
        AEVec2Set(&vec_a, 1, 0);
        AEVec2Rotate(&vec_b, &vec_a, rotation_per_part);
        for (s32 i = 0; i < parts; ++i)
        {
            AEGfxTriAdd(
                    0, 0, 0xFF000000, 0.0f, 1.0f,  // bottom-left: red
                    vec_a.x, vec_a.y, 0xFF000000, 0.0f, 0.0f,   // bottom-right: green
                    vec_b.x, vec_b.y, 0xFF000000, 0.0f, 0.0f);  // top-left: blue
                                                           //   
            AEVec2Rotate(&vec_a, &vec_a, rotation_per_part);
            AEVec2Rotate(&vec_b, &vec_b, rotation_per_part);
        }

        // Saving the mesh (list of triangles) in square_mesh
        circle_mesh = AEGfxMeshEnd();
    }
    ...
    while (gGameRunning)
    {
        ...
        // Collision
        if (circle_to_circle_collision(
            player_pos.x, player_pos.y, PLAYER_SIZE / 2,
            damage_tower_pos.x, damage_tower_pos.y, EFFECT_SIZE / 2))
        {
        ...
        if (circle_to_circle_collision(
            player_pos.x, player_pos.y, PLAYER_SIZE / 2,
            healing_tower_pos.x, healing_tower_pos.y, EFFECT_SIZE / 2))
        {
            player_hp += dt * EFFECT_SPEED;
            if (player_hp >= 100.f)
                player_hp = 100.f;
        ...
        // Draw damage tower
        draw_circle(damage_tower_pos.x, damage_tower_pos.y, EFFECT_SIZE/2, 1, 0, 0, 0.5f);

        // Draw healing tower
        draw_circle(healing_tower_pos.x, healing_tower_pos.y, EFFECT_SIZE/2, 0, 1, 0, 0.5f);

        // Draw Player
        draw_circle(player_pos.x, player_pos.y, PLAYER_SIZE/2, 0, 0, 1, 1); 
...
#if 0
            draw_rectangle(
                health_bar_x - outline/2, health_bar_y + outline/2,
                health_bar_width + outline, health_bar_height + outline,
                0, 0, 0, 1,
                0, 1);
#endif
            draw_rectangle(
...
#if 0
                draw_rectangle(
                    icon_x - outline / 2, icon_y + outline / 2,
                    icon_w + outline, icon_h + outline,
                    0, 0, 0, 1,
                    0, 1);
#endif
